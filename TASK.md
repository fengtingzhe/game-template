...
本模板支持同一项目下的多子对话协作。每个系统子对话可以在 `DESIGN_INBOX/<system_name>/` 下创建自己的工作区，其中 `schemes/` 用于保存好方案和备选方案，`version_plans/` 用于保存准备提交总控审核的版本方案。
...
- 根据用户确认结果更新 TASK.md。
- 如本轮来自系统子对话，只允许写入自己的 `DESIGN_INBOX/<system_name>/` 工作区。
...
- 不创建复杂配置表。
- 不读取 _archive/。
- 系统子对话不得把完整聊天记录、普通问答、重复解释写入 DESIGN_INBOX/。
- 系统子对话不得直接晋升内容到正式文件。
...
CHANGELOG.md
GPT_DEMO/
DESIGN_INBOX/<system_name>/schemes/，仅限系统子对话记录好方案时
DESIGN_INBOX/<system_name>/version_plans/，仅限系统子对话形成版本方案包后
...
- [x] 读取 _archive/
- [x] 系统子对话绕过总控直接修改正式文件
- [x] 系统子对话将普通问答或完整聊天记录写入 DESIGN_INBOX/
...
- 多子对话并行讨论时，每个子对话拥有自己的 `DESIGN_INBOX/<system_name>/` 工作区。
- 好方案、备选方案、设计思路写入 `schemes/`。
- 整理成包、准备给总控评审的版本方案写入 `version_plans/`。
- 总控默认审核 `version_plans/`，再决定是否晋升。
...
- 如来自系统子对话，内容只写入自己的 `DESIGN_INBOX/<system_name>/` 工作区，未直接污染正式文件。
...
TASK.md
CHANGELOG.md
DESIGN_INBOX/<system_name>/schemes/，如来自系统子对话的方案记录
DESIGN_INBOX/<system_name>/version_plans/，如来自系统子对话的版本方案
