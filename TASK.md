...
本模板支持同一项目下的多子对话协作。系统子对话平时只讨论，不默认写入仓库；只有形成版本方案包后，才写入 `DESIGN_INBOX/`，由总控对话在 `CONTROL_REVIEW.md` 中审核后，再决定是否晋升到正式文件。
...
- 根据用户确认结果更新 TASK.md。
- 如本轮来自系统子对话，只有形成版本方案包后，才允许写入 DESIGN_INBOX/。
...
- 不创建复杂配置表。
- 不读取 _archive/。
- 系统子对话不得把普通问答、零散灵感、未整理头脑风暴写入 DESIGN_INBOX/。
- 系统子对话不得直接晋升内容到正式文件。
...
- [x] 读取 _archive/
- [x] 系统子对话绕过总控直接修改正式文件
- [x] 系统子对话未形成版本方案包就写入 DESIGN_INBOX/
...
- 多子对话并行讨论时，子对话平时只讨论；形成版本方案包后再投递到 DESIGN_INBOX/；总控再审核晋升。
...
- 如来自系统子对话，只有版本方案包写入 DESIGN_INBOX/，未直接污染正式文件。
