...
只有当子对话形成一个可命名、可评审、可被总控分配的版本方案包时，才投递到：

```text
DESIGN_INBOX/
```

总控对话负责读取 `DESIGN_INBOX/`，并在 `CONTROL_REVIEW.md` 中判断是否采纳、暂缓、废弃或晋升到正式文件。
...
如果无法判断阶段、范围、权限、对话类型、是否已形成版本方案包或任务归属，AI 必须停止执行，不得自行猜测。
